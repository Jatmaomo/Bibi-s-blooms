 ...
 * Includes details of the product, clarifying all items are ready-to-wear garments (not sewing).
 */
export function getWhatsAppOrderUrl(
  ...
  message += `I would like to order this ready-to-wear piece from your collection:\n`;
  ...
  message += `\nPlease confirm availability and delivery. (Ready-to-wear piece, not sewing)`;
...
export function getWhatsAppContactUrl(customMessage?: string): string {
  const message =
    customMessage ||
    `Hi Bibi, I’d like to shop for some wears. Please send me your latest ready-to-wear catalog and available sizes.`;
  return `https://wa.me/${WHATSAPP_INTL}?text=${encodeURIComponent(message)}`;
}
