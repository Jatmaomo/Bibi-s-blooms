  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col ${isVertical ? 'items-center text-center' : 'text-left'}`}>
        <span
          className={`font-luxury uppercase text-[#f3f4f6] group-hover:text-[#c5a059] transition-colors leading-tight ${titleSizeClasses}`}
        >
          Bibi's Blooms
        </span>
        <span className="text-[10px] sm:text-xs text-zinc-400 font-semibold uppercase tracking-[0.25em] mt-0.5">
          Men's Fashion Wears
        </span>
        {showTagline && (
          <span className="text-[11px] sm:text-xs text-[#c5a059] font-medium tracking-wider italic mt-1 opacity-95">
            Love, Joy and Blooms... ♥
          </span>
        )}
      </div>
